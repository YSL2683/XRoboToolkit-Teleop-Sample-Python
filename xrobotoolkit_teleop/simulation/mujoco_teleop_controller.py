...
class MujocoTeleopController(BaseTeleopController):
    """
    Base class for teleoperation in MuJoCo simulation.
    """
    ...
    def __init__(
        ...
        xml_path: str,
        robot_urdf_path: str,
        manipulator_config: Dict[str, Any],
        floating_base: bool = False,
        R_headset_world: np.ndarray = np.eye(3),
    ...
    def _robot_setup(self):
        ...
        if self.mj_qpos_init is None:
            ...
            self.mj_data.qpos[:len(self.mj_qpos_init)] = self.mj_qpos_init
            self.mj_data.ctrl[:] = calc_mujoco_ctrl_from_qpos(self.mj_model, self.mj_data.qpos)
        ...
        for name, config in self.manipulator_config.items():
            ...
            if mocap_id != -1 and self.mj_model.body_mocapid[mocap_id] != -1:
                self.target_mocap_idx[name] = self.mj_model.body_mocapid[mocap_id]

    def _placo_setup(self):
        """
        Initialize Placo and sync its state to MuJoCo if initial qpos provided.
        """
        super()._placo_setup()
        
        if self.mj_qpos_init is not None:
            # Sync placo state to MuJoCo starting posture
            self.placo_robot.state.q = calc_placo_q_from_mujoco_qpos(
                self.mj_model, self.placo_robot, self.mj_data.qpos, self.floating_base
            )
    ...
    def _send_command(self):
        ...
        for gripper_name, gripper_target in self.gripper_pos_target.items():
            for joint_name, joint_pos in gripper_target.items():
                set_mujoco_joint_pos_by_name(self.mj_model, qpos_desired, joint_name, joint_pos)
